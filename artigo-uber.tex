...
%diferença monolitico e microserviço ---Documenting Software Architectures: Views and Beyond(2010)
Arquitetura de software tem emergido como uma importante pratica no desenvolvimento e na Engenharia de Software (Clements et al. 2010). De forma geral, arquitetura é uma divisão prudente do "todo" em partes, relacionando-as especificamente entre si e estabelecendo restrições nas atividades futuras. 

Aplicações podem ser classificadas como monolíticas, funcionalidades são agrupadas dentro de um único e grande sistema, minimizando a duplicidade de código e exigindo um único deploy. A arquitetura monolítica tem como desvantagem a falta de flexibilidade, escalabilidade limitada e principalmente a falhas, se um módulo estiver com erro todo o sistema cai, causando uma interdependia grande entre os módulos do sistema. Grandes soluções atualmente tem como alterativa a arquitetura de microserviços, onde os módulos são dependentes e não causam grande dano quanto um está fora do ar como no monolítico.
...
Também é questionável a afirmação de que é mais fácil criar uma solução nova do que utilizar uma solução existente por ser mais dificil de entender uma solução já criada e testada em toda sua planitude.
