...
Os artigos da Uber apresentados neste artigo, assim como outros no blog de Engenharia da Uber (https://eng.uber.com), fornecem uma visão única de como funciona
...
a maioria deste artigo, a Uber preferiu criar soluções próprias em cima de soluções e conceitos já existentes.
...
Também é possível questionar uma certa arrogância que os artigos implicam quando diversos engenheiros afirmam que para uma implementação ser feita na Uber, todos os componentes devem ser entendidos pelos desenvolvedores e
engenheiros de maneira exaustiva, e que em muitos casos era menos custoso criar uma implementação própria do que entender uma existente. Enquanto é afirmado que é mais fácil criar uma implementação totalmente nova, não é considerado que
a equipe da Uber participe do desenvolvimento e implementação de correções de \textit{bugs} de \textit{frameworks} e programas existentes. 
