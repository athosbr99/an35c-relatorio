...
\begin{resumo} 
  Esse artigo aborda sobre como a empresa Uber, hoje responsável por transporte de passageiros lida com o tratamento de dados nas 
  aplicações desenvolvidas, conhecendo sobre a metodologia, linguagens de desenvolvimento, \textit{frameworks} utilizados e como utilizam microserviços para gerenciar seus bancos de dados MySQL.
\end{resumo}

\section{Introdução}

A internet se tornou um fator primordial de uma sociedade e um fator critico no crescimento de uma nação. É possível afirmar que a sociedade atual vive na economia da informação e que
...
ou sistemas de \textit{middleware}. A arquitetura do sistema utilizando pela empresa deve ser condizente com o tamanho da empresa, carga de uso e numero de usuários, assim
como a importância das informações armazenadas. 

O sistema de banco de dados selecionado não pode demorar para entregar as informações requisitadas, muito menos perde-las.
...
sendo necessário descobrir novas formas de implementar uma estrutura de banco de dados confiável. Caso o sistema também tenha uma arquitetura monolítica e alguma parte deste
sistema tenha defeito, o resultado pode ser catastrófico para a sobrevivência da empresa.

\section{Objetivo}

Neste trabalho será relatado como a Uber, empresa pioneira em transporte publico compartilhado através de aplicativos, armazena suas informações 
através do conhecido SGBD (Sistema de Gerenciamento de Banco de Dados) MySQL da Oracle em uma arquitetura de 
...
A empresa norte-americana UberCab, fundada em 2009 por Garrett Camp e Travis Kalanick com a proposta inicial de ser um serviço semelhante a um táxi com carros de luxo pretos tendo as requisições feitas por um dispositivo eletrônico. Em maio de 2010 foi lançada a versão beta e somente em 2011 o serviço foi lançado oficialmente na cidade de São Francisco com preços mais caros que um táxi normal. 
...
Em 2013 o serviço já estava em 35 cidades. Hoje a Uber possui serviços de transportes, entregas de alimentos e pacotes, com colaboradores utilizando desde bicicletas até caminhões, presente em 633 cidades do mundo \cite{uber2017}.

Estudos realizados pelo IBGE afirma que entre os 12 milhões de trabalhadores desempregados têm como alternativa de trabalho o 
aplicativo, que passou de fevereiro de 2016 a janeiro de 2017 de 1 milhão de usuário para 8,7 milhões, ajudando assim os trabalhadores 
a usarem o aplicativo como renda extra e principalmente como trabalho principal gerando até 6 mil reais por mês de trabalho \cite{lisboa2017}. 

Atualmente existem 50 milhões de clientes usuários do aplicativo no mundo todo, dados apontam que no Brasil desde 2014 foram 
realizadas em torno de 530 milhões de viagens em todo o território nacional. Em 2016, a cidade de São Paulo superou a liderança da Cidade do México
...
A solução para este problema foi a criação de um design de sistema que foi dado o nome de \textit{Schemadock}, constituindo em utilizar diversos containers do MySQL
...
Entre o processo de download e de inicio do MySQL, é verificado se no volume montado para o \textit{container} existe algum dado (\url{https://docs.docker.com/engine/admin/volumes/volumes/}). 
Se não, caso seja uma instancia mestre, é iniciado um \textit{script} que coloca alguns usuários e tabelas padrões. Caso seja uma instancia
...
Os artigos da Uber são, talvez propositalmente, escassos em relação aos resultados das implementações. Porém, é possível listar algumas coisas.

Como resultado da implantação do Projeto \textit{Mezzanine} e do Docker, a Uber possui ferramentas de manutenção diária que possuem interfaces
para manipulação, sem necessidade de interagir diretamente com o hospedeiro dos bancos de dados. É estimado que existam mais de 1500 servidores 
do Docker com mais de 2300 bancos de dados MySQL \cite{uber2016-3}. 

O Projeto \textit{Mezzanine} permitiu que o armazenamento de viagens fosse dobrado mais uma vez sem o serviço ficar indisponível, além de várias
melhorias na performance e operacionais. O \textit{Schemaless} também foi implementado em outros serviços da Uber \cite{uber2015}.
...
Em \cite{uber2015}, é listada algumas dicas em relação a migração, tais como:
...
sendo inaceitável a perda de informações dentro de uma empresa, seja ela de grande ou pequeno porte. As requisições precisam ser 
executas com precisão e as respostas exibidas o mais rápido possível. Geralmente estudantes ou iniciantes na indústria não conhecem
como funciona um sistema de uma empresa grande e desconhecem que conceitos aplicados na prática podem divergir do que é ensinado na academia,
como é o caso das tecnologias e metodologias que a Uber utiliza para a criação de seus banco de dados e sua respectiva gestão. É importante que
recém-formados e novatos no geral tentem conhecer como funciona um banco de dados corporativo de empresas grandes, ou \textit{ex-startups}, no caso
da Uber, para ter uma melhor inserção de mercado. Também é importante que veteranos tenham conhecimento de novas técnicas e tecnologias para que
suas empresas continuem a ter práticas de desenvolvimento cada vez melhores, entregando cada vez mais valor aos seus consumidores.
