...
\begin{itemize}
  \item \textit{Container:} Um \textit{container} do Docker é um ambiente isolado, incluindo um sistema operacional compartilhado, \textit{middleware} e quaisquer
  serviços e arquivos necessários para funcionar uma aplicação dentro de um container.
  \item Imagem: É um modelo somente escrita, no qual os \textit{containers} são baseados. Uma imagem tem como base outra imagem de um sistema operacional, sendo colocado
  componentes adicionais para posterior distribuição. Exemplos de imagens são a do PostgreSQL (https://hub.docker.com/_/postgres/), NodeJS (https://hub.docker.com/_/node/)
  ...
  \item Dockerfile: É um arquivo texto que contem instruções a serem executadas em cima de uma imagem disponível no Docker Hub, a nuvem do Docker. Quando uma requisição de \textit{build}
  é executada com uma Dockerfile, o \textit{daemon} lê as instruções que estão no arquivo -- geralmente são comandos para serem executados no \item{shell} do sistema operacional do \textit{container} -- e
  retornando o \textit{container} criado.
  \item Docker Registry: Imagens do Docker são salvas em repositórios e compartilhadas. O Docker possui uma nuvem publica e gratuita para ser utilizada chamada Docker Hub.
